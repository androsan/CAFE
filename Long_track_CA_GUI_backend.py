"""  Program za nukleacijo in rast zrn po 'Cellular Automata Mesh Dependency' metodi v 3D,
      avtor:  dr. Andraž Kocjan, Inštitut za kovinske materiale in tehnologije, Oktober 2020  """
import os
import json
import numpy as np
...
root = Tk(); #root.geometry('1000x825+0+0');
root.title('SLM Microstructure Simulation')
...
'''...lables ............................................ '''
lab0 = Label(root, textvariable=var0, font=('Arial', '20'), bg='black', fg='white', height=2, width=13); lab0.grid(row=100, column=100, columnspan=2)
...
pick_selection_mechanism =         1            # pick 7 for fully random grains and directions;  5 for segmented directions
...
if avtomatska_nukleacija:
   ...
   if from_beginning:
      ...
      Negatives = {-1:0}; asc = {}; grain_counter = 0; inactive_grains=[]; S={}
...
if not avtomatska_nukleacija:
    ...
    Negatives = {-1:0};  S = {}                    
    asc = {}; grain_counter = 0; inactive_grains=[]; FF = []
...
def Run_CA(p):
    global tm_count, yp_count, run_ca, F, TC_list, nega, GG_list, i, MM_list,q,run_ca, h, grain_ID,Negatives, faza, cas, S, FF, vg, T_next, T, grain_counter, GDSM,  asc,  taula, likvid, R, cell, W,  random_distances, Selection
    i+=1
    step_time_start = time.time(); h+=1; var0.set(i)
    ...
    if run_3rd:
        if not MPI:
            ...
            for selsel in Selection:
                GG_partial=partial( mpf.GG, GDSM, faza, asc, vg, S, taula, T_next, T, likvid, R, cell, W, cas, random_distances)
                #GG_partial=partial(mpf.GG_, faza, S, cas)
                #GG_list = [GG_partial(selsel)]
                faza, cas = GG_partial(selsel)
    ...
    if i>=END_step:
        ...
        run_ca=root.after(200, lambda:Run_CA(p))
...
if __name__ == '__main__':
    ...
    # run the code without Tkinter GUI
    #for turbo in range(START_step, END_step+1):
     #   Run_CA(pool)

    # run the code with Tkinter GUI
    Run_CA(pool)
...
but0 = Button(root, text='RUN', command=lambda: Run_CA(pool), font=('Arial', 20), fg='green' ); but0.grid(row=101, column=100)
but1 = Button(root, text='Stop', command=lambda: Stop_CA(), font=('Arial', 20), fg='red'); but1.grid(row=101, column=101)
