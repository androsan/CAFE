...
def Load_NPY(yp, tr, x):
    npy = np.load(PATH+mapa+yp+'/'+tr+'/salome_'+str(x)+'.npy')
    return npy
...
#mapa=   'INTER_5_8  Z[0-8], X[15-27], Y[12-97], 1500°C/'
...
flashies_RGB =       track_main+'flashies_RGB/'                             #  Subfolder with time-snap 3D matrices, i.e. flashies
flashies_faza =        track_main+'flashies_faza/'

cuts_RGB =            track_main+'cuts_RGB/'                                  #  Subfolder with cut 3D matrices, i.e. cuts
cuts_faza =             track_main+'cuts_faza/'
'''--------------------------------------------------------------------------------------------------------------------------------------------'''

'''.....................................................................Side Tracks..................................'''
track_bottom =        'Track_1/'
cut_BOTTOM =       track_bottom+'cuts_faza/'                       #  "faza" field of bottom neighbour track

track_up = '...'
cut_UP =   '...'                                                                       #  "faza" field of upper neighbour track 
'''-----------------------------------------------------------------------------------------------------------'''
...
delete_inactive_grains_ID =         True     ; FF_length = 15     #  Inactive grains are deleted every FF_length time step

save_flashy_as_RGB =               True
save_flashy_as_faza =                True
...
dt = FEM_time_step / extra_time_factor


'''****************************'''
START_step =          319                     # Starting time step number
END_step     =          2500                    # Ending time step number
'''****************************'''
...
ß =     10                # case: SLM_2D_Source(cell/3)
#ß =   0.1


# ````` Supplemental variables, functions and calculated parameters `````
Negatives = {-1:0}; S = {}; np.random.seed(75689803)
...
if avtomatska_nukleacija:
    #faza = np.zeros((Z,X,Y))                               # fazna matrika, za track # 0
    #asc = {}; grain_counter = 0
    ...
    '''...............Loading asc from nuclei_data.json of previous track...............................'''
    
    with open(PATH+mapa+track_main+'nuclei_data.json', 'r') as nuks:
        asc = json.load(nuks)
    ...
    grain_counter = np.max(list(asc.keys()))

    IG = {int(keyy): [] for keyy, vall in asc.items()}
    AG = {int(keyy): [] for keyy, vall in asc.items()}
    
    FS_left   =  np.load(PATH+mapa+cut_BOTTOM+'cut_faza_'+str(yp_count)   +'.npy')[:,:X_cutoff,:]            # FS stands for "flashy snap"
    FS_right =  np.load(PATH+mapa+cut_BOTTOM+'cut_faza_'+str(yp_count+1)+'.npy')[:,:X_cutoff,:cutoff_limit]
...
FF = []
with open(PATH+mapa+track_main+'Logfile.txt', 'w')as cuttxt:
    cuttxt.write(100*'*'+'\n'+'z_min = '+str(z_min)+' ,  z_max = '+str(z_max)+' ,  Z = '+str(Z)+' ,\n'+
                 'X = '+str(X)+' ,  y_min = '+str(y_min)+' ,  y_max = '+str(y_max)+' ,  Y = '+str(Y)+' ,\n'+
                 'N = '+str(N)+' ,\n\n'+

                 'X_cutoff_percent = '+str(X_cutoff_percent)+' ,  X_cutoff = '+str(X_cutoff)+' ,  cutoff_limit = '+str(cutoff_limit)+' ,\n\n'+
...
start_time = time.time(); CT = [start_time] ; inactive_grains=[]
for i in range(START_step, END_step):
    ...
    if avtomatska_nukleacija:
        ...
        if np.all(taula[:,:,:cutoff_limit]==-1):
            print(); print(); print(50*'*',' CUT! ',50*'*')
            print('Cut_Off_Percent = ',Cut_Off_Percent,' %    , cutoff limit = ',cutoff_limit)
            CT.append(time.time())
            cut_time = CT[1] - CT[0] ; del CT[0]
            cut_time_hrs = str(int(cut_time/3600))
            cut_time_min = str(int((cut_time%3600)/60))
            cut_time_sec = str(int((cut_time%3600)%60))
            cut_text = '--- CUT # '+str(cut_count)+' --- @ time step:  '+str(i)+' ,    real time: '+str(round(1000*dt*i, 3))+' msec. ,    CPU time =  '+cut_time_hrs+'  hrs.   '+cut_time_min+'  min    '+cut_time_sec+'  sec.'
            print(cut_text)
            print(106*'*'); print()

            with open(PATH+mapa+track_main+'Logfile.txt', 'a')as cuttxt:
            ...
            faza_left = faza[:,:,cutoff_limit:]
            FS_right =  np.load(PATH+mapa+cut_BOTTOM+'cut_faza_'+str(yp_count+1)+'.npy')[:,:X_cutoff,:cutoff_limit]
            HA_right = np.zeros((Z,X-X_cutoff, Y-cutoff_limit))
            faza_right = np.hstack((HA_right,FS_right))
            
            faza = np.dstack((faza_left, faza_right))
    ...
    for g in grain_ID:
           ...
           with open(PATH+mapa+track_main+'nuclei_data_tracks_(0,1).json', 'w') as nuks:              # Writing data of ALL nuclei as .json file, but values must be list NOT np.array !!!
